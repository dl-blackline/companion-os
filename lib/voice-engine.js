 ...
 *   - OpenAI realtime voice mode (gpt-4o-realtime-preview) for low-latency
...
/**
 * Process voice input using OpenAI realtime model (gpt-4o-realtime-preview).
 *
...
async function processRealtimeVoice({ text, systemPrompt, voice }) {
  ...
  const realtimeModel = MODEL_CONFIG.voice;
  const realtimeVoice = voice || "alloy";
  ...
  const response = await client.chat.completions.create({
    ...
    audio: { voice: realtimeVoice, format: "wav" },
  });
 ...
 *
 * @param {object} params
 * @param {string} [params.model]  - Realtime model (default: gpt-4o-realtime-preview).
 * @param {string} [params.voice]  - Realtime voice (e.g. alloy, nova, aria, verse).
 * @returns {Promise<{client_secret: string, session_id: string}>}
 */
