...
async function retrieveContext({ embedding, user_id, conversation_id, getRecentConversation }) {
  const [
    ...
    getPersonalityProfile(user_id),
    getRecentEmotionalSignals(user_id),
    getRelationshipEvents(user_id),
  ]);
...
async function getFinancialHealthContext(user_id) {
  if (!user_id) {
  ...
  const [accountsRes, txRes] = await Promise.all([
    ...
    supabase
      ...
      .select("amount, pending, transaction_date")
      .eq("user_id", user_id)
  ...
  if (accounts.length === 0) {
    return "No linked financial accounts found.";
  }
  ...
  const validTx = transactions.filter((tx) => !tx.pending);
  const income30d = validTx
    .filter((tx) => toNumber(tx.amount) < 0)
    .reduce((sum, tx) => sum + Math.abs(toNumber(tx.amount)), 0);
  const expenses30d = validTx
    .filter((tx) => toNumber(tx.amount) > 0)
 ...
 */
async function executeTool(step, { message, context, user_id }) {
  switch (step.tool) {
    case "memory_search":
      return {
        tool: "memory_search",
    ...
    case "web_search": {
      try {
        ...
        return {
          tool: "web_search",
          result: formatSearchResults(message, results),
        };
      } catch (err) {
    ...
    case "maps_lookup": {
      try {
        // Attempt to extract a location/address from the message for geocoding first
        const geocoded = await geocodeAddress(message);
        if (geocoded) {
          return {
            tool: "maps_lookup",
            result: formatGeocodingResult(message, geocoded),
          };
        }

        // Fallback: search for places matching the message
        const places = await searchPlaces(message);
        return {
          tool: "maps_lookup",
          result: formatPlacesResults(message, places),
        };
      } catch (err) {
    ...
    case "financial_health_lookup": {
      try {
        return {
          tool: "financial_health_lookup",
          result: await getFinancialHealthContext(user_id),
        };
      } catch (err) {
        console.error("Financial health lookup error:", err.message);
        return {
          tool: "financial_health_lookup",
          result: "Financial health lookup failed: " + err.message,
        };
      }
    }
...
export async function orchestrate({
  ...
  for (const step of executionPlan.steps || []) {
    const result = await executeTool(step, { message, context, user_id });
  ...
  const unfiltered = isUnfilteredModel(model);
  ...
  // 8. Critic agent (skipped for unfiltered models to preserve the raw response;
  //    hard boundary enforcement is handled by the system prompt itself).
  const finalResponse = unfiltered
    ? assistantResponse
