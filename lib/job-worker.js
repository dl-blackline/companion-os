...
/**
 * Build the prompt object for an agent.
 */
function buildAgentPrompt(agent, taskDescription) {
  return {
...
/**
 * Parse an AI response as JSON, falling back to a wrapper object.
 */
...
async function handleAgentTask(payload) {
  ...
  try {
    const prompt = buildAgentPrompt(agent, task.task_description);
