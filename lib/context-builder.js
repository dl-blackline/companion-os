 ...
 *
 * Enhanced to include media memory context, support memory obedience,
 * and inject companion engine context (goals, constraints, initiatives).
 *
 * @param {object} params
...
export async function buildContext({ embedding, user_id, conversation_id, message, getRecentConversation, unfiltered, aiMood, customInstructions }) {
  const [
    ...
    message
      ...
      : Promise.resolve(""),
    // Retrieve companion engine context (goals, constraints, initiatives)
    buildCompanionContext(user_id).catch((err) => {
      console.error("Context builder – companion engine error:", err.message);
  ...
  const companionContextFormatted = formatCompanionContext(companionContext);
  ...
  return {
    ...
    relationshipContext,
    mediaMemoryContext,
    companionContext: companionContextFormatted,
 ...
 * and companion engine context (goals, constraints, initiatives).
 */
export function buildSystemPromptFromContext(context) {
  return buildSystemPrompt({
    ...
    companionContext: context.companionContext,
