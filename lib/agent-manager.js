 ...
 * @param {string|null} user_id - Owner of the task.
 * @returns {object|null} The created task record, or null on error.
 */
export async function createAgentTask(agent_type, task_description, user_id = null) {
 ...
 * @param {string|null} [user_id=null] - Optional owner filter.
 * @returns {Array} Array of agent task records.
 */
export async function getAgentTasksByStatus(status, limit = 20, user_id = null) {
  let query = supabase
    .from("agent_tasks")
    .select("*")
    .eq("status", status)
 ...
 */
export async function getAgentTask(task_id, user_id = null) {
  let query = supabase
    .from("agent_tasks")
    .select("*")
    .eq("id", task_id);
