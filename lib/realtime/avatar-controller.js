 ...
 *
 * @param {{ state: string, transitionState: string, expression: string, lipSyncFrames: any[], idleIntensity: number } | null} current
 ...
 *
 * @param {string | null} text - The text being spoken.
...
export function generateLipSyncFrames(text, durationMs = 3000) {
  ...
  // Frame interval (~80 ms) approximates the typical viseme duration in
  // real TTS engines, producing smooth mouth-movement updates at ~12.5 fps.
  const LIP_SYNC_FRAME_INTERVAL_MS = 80;

  // Base mouth-open values per phoneme category.  These are heuristic
  // stand-ins until real viseme data is available from the TTS engine.
  const VOWEL_MOUTH_OPEN_BASE = 0.6;
  const CONSONANT_MOUTH_OPEN_BASE = 0.2;
  const MOUTH_OPEN_VARIATION = 0.3;
  const SILENCE_MOUTH_OPEN = 0.1;

  const frameCount = Math.max(1, Math.floor(durationMs / LIP_SYNC_FRAME_INTERVAL_MS));
  ...
  for (let i = 0; i < frameCount; i++) {
    ...
    let mouthOpen;
    if (isSilence) {
      mouthOpen = SILENCE_MOUTH_OPEN;
    } else if (isVowel) {
      mouthOpen = VOWEL_MOUTH_OPEN_BASE + Math.random() * MOUTH_OPEN_VARIATION;
    } else {
      mouthOpen = CONSONANT_MOUTH_OPEN_BASE + Math.random() * MOUTH_OPEN_VARIATION;
    }

    frames.push({
      timeMs: i * LIP_SYNC_FRAME_INTERVAL_MS,
 ...
 *
 * @param {{ state: string, transitionState: string, expression: string, lipSyncFrames: any[], idleIntensity: number }} avatarState
 * @param {Array<{ timeMs: number, mouthOpen: number, viseme?: string }> | null} frames
 * @returns {{ state: string, transitionState: string, expression: string, lipSyncFrames: Array, idleIntensity: number }}
 */
