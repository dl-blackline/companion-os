...
import { generateLeonardoImage } from "./media/leonardo-image-generator.js";
...
import {
  generateImage as nofilterGenerateImage,
  generateVideo as nofilterGenerateVideo,
...
async function routeImage(entry, prompt, options) {
  switch (entry.provider) {
    ...
    case "leonardo":
      return generateLeonardoImage(prompt, {
        ...options,
        ...(entry.modelId ? { modelId: entry.modelId } : {}),
      });

    case "nofilter":
      return nofilterGenerateImage(prompt);
...
async function routeVideo(entry, prompt, options) {
  switch (entry.provider) {
    ...
    case "kling":
      return generateKlingVideo(prompt, {
        ...options,
        ...(entry.modelId ? { model: entry.modelId } : {}),
      });

    case "hailuo":
      return generateHailuoVideo(prompt, {
        ...options,
        ...(entry.modelId ? { model: entry.modelId } : {}),
      });
    ...
    case "nofilter":
      return nofilterGenerateVideo(prompt);
 ...
 * @param {string} params.prompt - Generation prompt
 * @param {string} [params.model] - Optional model id for the selected media type
 * @param {object} [params.options] - Additional provider-specific options
 * @returns {Promise<{type: string, model: string, provider: string, prompt: string, url?: string, taskId?: string}>}
 */
export async function runMediaTask({ type, prompt, model, options = {} }) {
  // Delegate all media types (including image) to the full media engine so
  // model/provider selection is applied consistently.
